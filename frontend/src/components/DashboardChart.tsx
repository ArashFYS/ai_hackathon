...
export default function DashboardChart({ data, scope }: { data: DashboardData; scope: DashboardScope }) {
  const t = useT()
  const { lang } = useLang()
  const [mode, setMode] = useState<'activities' | 'status'>('activities')
  const segments = mode === 'status' ? STATUS_CODES.map((status) => ({
    ...
    color: sector.sector === 'onbekend' ? '#ded9de' : PALETTE[i % PALETTE.length],
    ...
    <section className="dash-panel dash-distribution">
      <div className="dash-panel-heading">
        ...
        <div className="dash-tabs" role="group" aria-label={t('dashboard.distribution')}>
          {(['activities', 'status'] as const).map((tab) => <button key={tab} type="button" aria-pressed={mode === tab}
      ...
      <div className="dash-chart-body">
          ...
          <svg viewBox="0 0 180 180" aria-hidden="true">
            ...
            {segments.map((segment, i) => {
              ...
              return length > 0 && <circle key={segment.key} cx="90" cy="90" r="72" fill="none" stroke={segment.color}
                strokeWidth="22" strokeDasharray={`${Math.max(0, length - (segments.length > 1 ? 1.5 : 0))} ${circumference}`}
        ...
        <ul className="dash-chart-legend">
          {segments.map((segment) => <li key={segment.key}><Link to={segment.to}>
      ...
      <p className="dash-note">{t(mode === 'activities' ? 'dashboard.sectorNote' : 'dashboard.statusNote')}</p>
    </section>
  )
}
