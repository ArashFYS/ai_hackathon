...
import { decideProposal, kindLabel, proposalStatusLabel, valueLabel } from '../api'
...
export function ProposalStatusChip({ status }: { status: ProposalStatus }) {
  ...
  return (
    <span className={`inline-block whitespace-nowrap rounded px-2 py-0.5 text-xs font-medium ${CHIP[status] ?? CHIP.open}`}>
      {proposalStatusLabel(lang, status)}
    </span>
  )
}
...
export function DecideButtons({ proposal, onDecided, size = 'sm' }: { proposal: Proposal | null; onDecided: () => void; size?: 'sm' | 'xs' }) {
  ...
  const title = !proposal || proposal.status !== 'open' ? t('proposal.noOpen') : undefined
  ...
  return (
    <span className="inline-flex items-center gap-1">
      <button type="button" title={title} disabled={disabled} onClick={() => decide('bevestigd')}
        ...
        {busy === 'bevestigd' ? '…' : t('proposal.confirm')}
      </button>
      <button type="button" title={title} disabled={disabled} onClick={() => decide('afgewezen')}
        ...
        {busy === 'afgewezen' ? '…' : t('proposal.reject')}
      </button>
      {error && <span className="text-xs text-red-700">{t('proposal.failed')}</span>}
    </span>
  )
}

/** `p.reason` is backend/officer free text: shown as delivered. */
export default function ProposalList({ proposals, onChanged }: { proposals: Proposal[]; onChanged: () => void }) {
  const t = useT()
  const { lang } = useLang()
  if (proposals.length === 0) return <p className="text-sm text-gray-500">{t('proposal.none')}</p>
  return (
        ...
        <li key={p.id} className="flex flex-wrap items-start justify-between gap-2 py-2">
          <div className="min-w-0 flex-1">
            <div className="flex items-center gap-2">
              <span className="text-xs text-gray-500">{kindLabel(lang, p.kind)}{p.field ? ` · ${p.field}` : ''}</span>
              <ProposalStatusChip status={p.status} />
            </div>
            <p className="text-gray-800">{p.reason}</p>
            {(p.current_value || p.proposed_value) && (
              <p className="text-xs text-gray-600">
                <span className="line-through">{valueLabel(lang, p.current_value)}</span> → <span className="font-medium">{valueLabel(lang, p.proposed_value)}</span>
              </p>
            )}
            <p className="text-xs text-gray-400">
              {t('proposal.created', { date: p.created_at?.slice(0, 10) ?? '—' })}{p.decided_at ? t('proposal.decided', { date: p.decided_at.slice(0, 10) }) : ''}
            </p>
          </div>
