...
## Amendment (TICKET-030, merge onto main)

The visual theme (palette, logo, semantic layout classes, `data-status` badges) was merged onto the i18n/feature main. Not adopted from this branch: hardcoded English strings (the NL default + EN toggle from TICKET-029 on main stays the source of all UI text), the removal of leaflet (the Kaart page stays), the select-based source picker (the 7-tab minibrowser stays), and `ConfidenceScore` / `mapLocation.ts` are shipped as files but not wired in (Zekerheid badge stays; no numeric score in the API).
